...
public enum MadeProductType implements Product {
    ...
    JUICE("juice", CraftType.KEG,"A sweet, nutritious beverage.", null, new TimeAndDate(4, 0), null, null, true) {
        @Override
        public Integer calcPrice(Salable product) {
    ...
    WINE("wine", CraftType.KEG,"Drink in moderation.", null, new TimeAndDate(7, 0), null, null, true) {
        @Override
        public Integer calcEnergy(Salable product) {
...
//    },
    ...
    PICKLES("pickles", CraftType.PRESERVES_JAR, "A jar of your home-made pickles.", null, new TimeAndDate(0, 6), null, null, true) {
        @Override
        public Integer calcEnergy(Salable product) {
            return (int) (1.75 * (double) ((HarvestAbleProduct) product).getContainingEnergy());
        }
    ...
    JELLY("jelly", CraftType.PRESERVES_JAR,"Gooey.", null, new TimeAndDate(3, 0), null, null, true) {
        @Override
        public Integer calcEnergy(Salable product) {
            return 2 * ((HarvestAbleProduct) product).getContainingEnergy();
        }
    ...
    SMOKED_FISH("smoked fish",CraftType.FISH_SMOKER, "A whole fish, smoked to perfection.", null, new TimeAndDate(0, 1), null, null, true) {
        @Override
        public Integer calcEnergy(Salable product) {
    ...
    MadeProductType(String name, CraftType craftType, String description, Integer energy, TimeAndDate processingTime, Map<Salable, Integer> ingredients, Integer sellPrice, boolean isEdible) {
        this.name = name;
    ...
    MadeProductType(String name, CraftType craftType, String description, Integer energy, TimeAndDate processingTime, Map<Salable, Integer> ingredients, Integer sellPrice, boolean isEdible, boolean isCoalNeeded) {
        this.name = name;
        this.description = description;
        this.energy = energy;
        this.sellPrice = sellPrice;
    ...
    public static MadeProductType findByCraft(CraftType craft) {
        for (MadeProductType value : MadeProductType.values()) {
            if (craft.equals(value.getCraftType())) return value;
        }
        return null;
    }
    ...
    public Response isIngredientsValid(Salable product, Integer amount, boolean coal) {
        ...
        for (Salable ingredient : ingredients.keySet()) {
            if (product.equals(ingredient)) {
                if (amount >= ingredients.get(ingredient)) return new Response("", true);
    ...
    public Integer countIngredient() {
        return ingredients.values().iterator().next();
    }
