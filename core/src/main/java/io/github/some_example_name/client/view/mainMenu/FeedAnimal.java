...
public class FeedAnimal extends PopUp {
    ...
    public void createMenu(Stage stage, Skin skin, WorldController playerController) {
        ...
        Table feed = new Table();
        feed.top().left().pad(10).defaults().pad(5).left();
        feed.add(new Image(GameAsset.HAY_HOPPER_FULL)).row();
        feed.add(new Label("You feed a " + animal.getAnimalType().getName(), skin)).row();
        feed.add(new Label("Animal name: " + animal.getName(), skin)).padRight(20);
        feed.add(new Image(animal.getAnimalType().getTexture())).row();
        ...
        content.add(feed).expand().pad(10);
