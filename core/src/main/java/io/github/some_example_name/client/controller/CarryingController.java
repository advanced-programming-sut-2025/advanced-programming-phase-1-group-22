...
public class CarryingController {
    ...
    public void update() {
        for (Player player : App.getInstance().getCurrentGame().getPlayers()) {
            ...
            if (carryingObject != null && !(carryingObject instanceof Tool)) {
                Sprite sprite = null;
                if (carryingObject.getSprite() == null) {
                    if (carryingObject.getSprites() != null) {
                        sprite = carryingObject.getSprites().get(0).getSprite();
                    }
                } else {
                    sprite = carryingObject.getSprite();
                }
                if (sprite != null) {
