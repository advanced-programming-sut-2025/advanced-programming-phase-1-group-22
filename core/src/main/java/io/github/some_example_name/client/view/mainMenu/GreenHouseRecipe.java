...
public class GreenHouseRecipe extends PopUp {
    @Override
    public void createMenu(Stage stage, Skin skin, WorldController playerController) {
        ...
        info.top().left().pad(10).defaults().pad(5).left();
        info.add(new Label("need to build: ", skin)).row();
        info.add(new Label("    - 1000 Gold", skin)).row();
        info.add(new Label("    - 500 wood", skin)).row();
        info.add(new Label("click on greenhouse to build", skin)).row();
        ...
        content.add(info).padTop(20).expand().top();
