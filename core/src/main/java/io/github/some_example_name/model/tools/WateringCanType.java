...
public enum WateringCanType implements Tool {
    NORMAL("normal wateringCan", 0, 40, 5, 0,new Sprite(GameAsset.WATERING_CAN)),
    COPPER("copper wateringCan", 1, 55, 4, 2000 / 2,new Sprite(GameAsset.COPPER_WATERING_CAN)),
    IRON("iron wateringCan", 2, 70, 3, 5000 / 2,new Sprite(GameAsset.STEEL_WATERING_CAN)),
    GOLD("gold wateringCan", 3, 85, 2, 10000 / 2,new Sprite(GameAsset.GOLD_WATERING_CAN)),
    IRIDIUM("iridium wateringCan", 4, 100, 1, 25000 / 2,new Sprite(GameAsset.IRIDIUM_WATERING_CAN));
    ...
    WateringCanType(String name, Integer level, Integer capacity, Integer energyCost, int price, Sprite sprite) {
        this.name = name;
        this.level = level;
